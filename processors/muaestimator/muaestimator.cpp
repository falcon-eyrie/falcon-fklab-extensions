...
void MUAEstimator::CreatePorts() {
    data_in_port_ = create_input_port<SpikeType>(SPIKEDATA, SpikeType::Capabilities(),
                                                 PortInPolicy(SlotRange(1, 64)));

    data_out_port_ =
        create_output_port<MUAType>("mua", MUAType::Parameters(), PortOutPolicy(SlotRange(1)));

    bin_size_ = create_static_state(BIN_SIZE, initial_bin_size_(), true, Permission::WRITE);
...
void MUAEstimator::Prepare(GlobalContext& context) {
    ...
    if (data_in_port_->number_of_slots() > 1) {
        for (SlotType s = 1; s < data_in_port_->number_of_slots(); ++s) {
            if (spike_buffer_size_ != data_in_port_->prototype(s).buffer_size()) {
                throw ProcessingConfigureError("Incoming SpikeData buffer-sizes are different.",
                                               name());
            }
        }
    }
    ...
    try {
        ...
        check_buffer_sizes_and_log(spike_buffer_size_, x, true, n_spike_buffers_, name());
        initial_bin_size_ = x;
    } catch (std::runtime_error& error) {
        throw ProcessingStreamInfoError(error.what(), name());
    }
    LOG(INFO) << name() << ". MUA will be computed using " << n_spike_buffers_ << " spike buffers.";
