...
void SpikeDetector::CreatePorts() {
    ...
    data_out_port_spikes_ =
        create_output_port<SpikeType>(SPIKEDATA, SpikeType::Parameters(buffer_size_()),
                                      PortOutPolicy(SlotRange(1), RINGBUFFER_SIZE));

    data_out_port_events_ = create_output_port<EventType>(EVENTDATA, EventType::Parameters(),
                                                          PortOutPolicy(SlotRange(1)));

    threshold_ = create_static_state(THRESHOLD, initial_threshold_(), true, Permission::WRITE);

    peak_lifetime_ =
        create_static_state(PEAK_LIFETIME, initial_peak_lifetime_(), true, Permission::WRITE);
}
...
void SpikeDetector::CompleteStreamInfo() {
    ...
    try {
        ...
        check_buffer_sizes_and_log(incoming_buffer_size_ms, tmp, strict_time_bin_check_(),
                                   n_incoming_, name());
        buffer_size_ = tmp;
...
void SpikeDetector::Prepare(GlobalContext& context) {
    spike_detector_.reset(new dsp::algorithms::SpikeDetector(n_channels_, initial_threshold_(),
                                                             initial_peak_lifetime_()));

    if (invert_signal_()) {
        inverted_signals_.reset(
            new TimeSeriesType<double>::Data(data_in_port_->prototype(0).parameters()));
    }
}
...
void SpikeDetector::Process(ProcessingContext& context) {
    ...
    while (!context.terminated()) {
        ...
        while (sample_buffer_counter < n_incoming_) {
            ...
            // make a local copy of the inverted signal and use it for spike
            // detection
            if (invert_signal_()) {
                for (size_t sample = 0; sample < incoming_buffer_size_samples_; ++sample) {
                    for (unsigned int channel = 0; channel < n_channels_; ++channel) {
                        inverted_signals_->set_data_sample(sample, channel,
                                                           -data_in_->data_sample(sample, channel));
                    }
                }
            ...
            for (size_t sample = 0; sample < incoming_buffer_size_samples_; ++sample) {
                if (spike_detector_->is_spike<double*>(data_in_->sample_timestamp(sample),
                                                       signals->begin_sample(sample))) {
                    spike_data_out_->add_spike(spike_detector_->amplitudes_detected_spike(),
