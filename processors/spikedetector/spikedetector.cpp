...
void SpikeDetector::CreatePorts() {
    ...
    data_out_port_events_ = create_output_port<EventType>(
        EVENTDATA, EventType::Parameters(), PortOutPolicy(SlotRange(1)));

    threshold_ = create_static_state(THRESHOLD, initial_threshold_(), true,
                                     Permission::WRITE);

    peak_lifetime_ = create_static_state(
        PEAK_LIFETIME, initial_peak_lifetime_(), true, Permission::WRITE);
}
...
void SpikeDetector::CompleteStreamInfo() {
    ...
    double incoming_buffer_size_ms = incoming_buffer_size_samples_ /
    ...
    try {
        ...
        check_buffer_sizes_and_log(incoming_buffer_size_ms, tmp,
                                   strict_time_bin_check_(), n_incoming_,
                                   name());
        buffer_size_ = tmp;
    ...
    data_out_port_spikes_->streaminfo(0).set_stream_name(
        data_in_port_->streaminfo(0));

    data_out_port_events_->streaminfo(0).set_stream_name(
        data_in_port_->streaminfo(0));
        ...
        while (sample_buffer_counter < n_incoming_) {
            ...
            if (invert_signal_()) {
                for (size_t sample = 0; sample < incoming_buffer_size_samples_;
                     ++sample) {
                    for (unsigned int channel = 0; channel < n_channels_;
                         ++channel) {
...
void SpikeDetector::Postprocess(ProcessingContext &context) {
    LOG(INFO) << name()
              << ". # spikes detected = " << spike_detector_->nspikes();
