...
void EventFilter::Preprocess(ProcessingContext& context) {
    ...
    if (blockout_time_() > 0) {
        std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(blockout_time_())));
    }
    n_blocked_events_ = 0;
}
...
void EventFilter::Process(ProcessingContext& context) {
    ...
    std::vector<uint64_t> arrival_hwTS_per_slot_blocking_events(block_in_port_->number_of_slots(),
                                                                0);
    ...
    while (!context.terminated()) {
        if (!detection_criterion) {
            ...
            std::tie(alive, event_received, slot_last) =
                is_there_target(data_in_port_, event_counter_, arrival_times_per_slot_events,
            ...
            if (event_received) {
                ...
                for (auto t : arrival_times_per_slot_events) {
                    if (time_between(arrival_times_per_slot_events[slot_last], t) < sync_time_()) {
            ...
            std::tie(alive, detection_block, std::ignore) = is_there_target(
                block_in_port_, blocking_events_counter_, arrival_times_per_slot_blocking_events,
        ...
        if (detection_criterion) { // check again as flag might have just
            ...
            if (time_since(gate_close_time_) <= blockout_time_()) {
                ...
                // is received on the "events" port with this dedicated read
                // loop read incoming blocking events for block_wait_time_ms_
                while (time_since(t_detection) < block_wait_time_() && detection_criterion) {
...
std::tuple<bool, bool, std::size_t> EventFilter::is_there_target(
    ...
    for (decltype(input_port->number_of_slots()) s = 0; s < input_port->number_of_slots(); ++s) {
        ...
        if (nread > 1 && !discard_warnings_()) {
            ...
            LOG(WARNING) << name() << ". " << nread - 1 << " events on port " << input_port->name()
                         << "(" << events_list << ") were discarded.";
        }
        ++event_counter.all_received;

        // if there's data, check if it is a target event
        if (*data_in.back() == target_event_()) {
            LOG(DEBUG) << name() << ". Received target event " << target_event_().event()
