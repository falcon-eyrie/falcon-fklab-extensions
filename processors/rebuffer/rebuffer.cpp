...
void Rebuffer::CompleteStreamInfo() {
    ...
    if (data_in_port_->number_of_slots() != data_out_port_->number_of_slots()) {
        throw ProcessingStreamInfoError("Number of outputs does not match the number of inputs.",
                                        name());
    }
    ...
    if (buffer_size_.unit() == units::precise::sample_units && buffer_size_() > 0.) {
        ...
        for (int k = 0; k < data_in_port_->number_of_slots(); ++k) {
            sample_buffer_[k] = time2samples<unsigned int>(
                buffer_size_(), data_in_port_->prototype(k).sample_rate() / downsample_factor_());
        ...
        for (int k = 0; k < data_in_port_->number_of_slots(); ++k) {
            sample_buffer_[k] = std::max(
                1u, static_cast<unsigned int>(
                        std::floor(data_in_port_->prototype(k).nsamples() / downsample_factor_())));
        }
    }

    // finalize
    for (int k = 0; k < data_in_port_->number_of_slots(); ++k) {
        data_out_port_->streaminfo(k).set_parameters(TimeSeriesType<double>::Parameters(
            data_in_port_->prototype(k).ncolumns(), sample_buffer_[k],
            data_in_port_->prototype(k).sample_rate() / downsample_factor_()));
