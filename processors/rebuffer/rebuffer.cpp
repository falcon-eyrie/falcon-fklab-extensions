...
void Rebuffer::CompleteStreamInfo() {
    ...
    if (buffer_size_.unit() == units::precise::sample_units &&
        ...
        for (int k = 0; k < data_in_port_->number_of_slots(); ++k) {
            sample_buffer_[k] = time2samples<unsigned int>(
                buffer_size_(), data_in_port_->prototype(k).sample_rate() /
                                    downsample_factor_());
            if (sample_buffer_[k] == 0) {
                throw ProcessingStreamInfoError("Buffer duration is zero.",
                                                name());
            }
        }
    } else {
    ...
    for (int k = 0; k < data_in_port_->number_of_slots(); ++k) {
        ...
        data_out_port_->streaminfo(k).set_stream_name(
            data_in_port_->streaminfo(k).stream_name());
            ...
            while (s < data_in->nsamples()) {
                for (s = offset[k]; s < data_in->nsamples() &&
                    ...
                    for (unsigned int c = 0; c < data_in->ncolumns(); ++c) {
                        data_out[k]->set_data_sample(
                            sample_out_counter[k], c,
                            data_in->data_sample(s, c));
                    }
                    data_out[k]->set_sample_timestamp(
                        sample_out_counter[k], data_in->sample_timestamp(s));
