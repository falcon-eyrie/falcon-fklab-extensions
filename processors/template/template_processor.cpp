...
void FalconProcessor::CreatePorts() {
    ...
    data_out_port_ = create_output_port("data", AnyDataType(),
                                        PortOutPolicy(SlotRange(m, n)));

    other_data_out_port_ = create_output_port("other_data", AnyDataType(),
                                              PortOutPolicy(SlotRange(m, n)));

    state_variable1_ = create_writable_shared_state("state_variable1",
                                                    default_state_variable1_,
                                                    Permission::, Permission::);

    state_variable2_ = create_readable_shared_state("state_variable2",
                                                    default_state_variable2_,
                                                    Permission::, Permission::);
}
...
void FalconProcessor::Postprocess(ProcessingContext &context) {
    LOG(INFO) << name() << ". STREAMED "
              << data_port_->slot(0)->nitems_produced() << " data packets";
}
