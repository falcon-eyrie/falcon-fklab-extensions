...
void FalconProcessor::CreatePorts() {
    data_in_port_ = create_input_port("data", AnyDataType(), PortInPolicy(SlotRange(m, n)));

    other_data_in_port_ =
        create_input_port("other_data", AnyDataType(), PortInPolicy(SlotRange(m, n)));

    data_out_port_ = create_output_port("data", AnyDataType(), PortOutPolicy(SlotRange(m, n)));

    other_data_out_port_ =
        create_output_port("other_data", AnyDataType(), PortOutPolicy(SlotRange(m, n)));

    state_variable1_ = create_writable_shared_state("state_variable1", default_state_variable1_,
                                                    Permission::, Permission::);

    state_variable2_ = create_readable_shared_state("state_variable2", default_state_variable2_,
                                                    Permission::, Permission::);
}
...
void FalconProcessor::Process(ProcessingContext& context) {
    ...
    while (!context.terminated()) {
        ...
        // clearing will take an extra operation, don't clear if you are going
        // to overwrite
