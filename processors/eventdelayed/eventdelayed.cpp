...
EventDelayed::EventDelayed() : delayed_range_(150, 200) {
    ...
    add_option(
        "delayed range", initial_delayed_range_,
    ...
    add_option(STOP_ANALYSIS_TIME_S + "/starting time",
               when_stop_analysis_period_,
    ...
    add_option(STOP_DETECTION_TIME_S + "/period",
               initial_stop_detection_period_,
...
void EventDelayed::Configure(const GlobalContext &context) {
    ...
    if (initial_stop_detection_period_() == 0) {
        ...
        LOG(INFO) << name() << ". Max output frequency set to "
                  << 1e3 / static_cast<double>(initial_stop_detection_period_())
                  << " Hz.";
    }
    ...
    if (initial_stop_analysis_period_() == 0) {
        ...
        LOG(INFO) << name() << ". Analysis time off after stimulation set to "
                  << static_cast<double>(initial_stop_analysis_period_())
                  << " s.";
    }
    ...
    if (!initial_delayed_event_()) {
        ...
        LOG(INFO) << name() << ". Events are delayed of a range between"
                  << delayed_range_.lower() << " and " << delayed_range_.upper()
                  << " ms.";
    }

    if (delayed_range_.upper() - delayed_range_.lower() <
        initial_stop_detection_period_()) {

        LOG(INFO) << name()
                  << ". When the difference between maximal and minimal "
                     "possible delay is below the lockout period,"
                  << "some weird behavior of detection locked out or not "
                     "locked out when it should, could occur.";
    }
}

void EventDelayed::CreatePorts() {
    data_in_port_ = create_input_port<EventType>(
        EventType::Capabilities(), PortInPolicy(SlotRange(1), false));

    output_port_ = create_output_port<EventType>(
        EventType::Parameters(DEFAULT_EVENT), PortOutPolicy(SlotRange(1)));
    // -----  Mode state --- //
    disabled_ = create_follower_state(DISABLED_S, default_disabled_(),
                                      Permission::WRITE);

    delayed_event_ = create_follower_state(DELAYED_S, initial_delayed_event_(),
                                           Permission::WRITE);

    // ----- Lock-out state --- //
    stop_detection_period_ = create_static_state(
        STOP_DETECTION_TIME_S, initial_stop_detection_period_(), true,
        Permission::WRITE);

    stop_analysis_period_ = create_static_state(STOP_ANALYSIS_TIME_S,
                                                initial_stop_analysis_period_(),
                                                true, Permission::WRITE);

    analysis_unlocked_ = create_broadcaster_state(
        "analysis enabled", true,
        Permission::READ); // connected to the ripple detection processor to
                           // disable/enabled ripple processing
}

void EventDelayed::CompleteStreamInfo() {
    output_port_->streaminfo(0).set_stream_name(
...
void EventDelayed::Process(ProcessingContext &context) {
    ...
    while (!context.terminated()) {
        ...
        while (!lockout_queue_.empty() and
               lockout_queue_.top().ts < Clock::now()) {

            LOG(DEBUG) << name()
                       << ". Start a lockout after stimulation for " +
                              std::to_string(
                                  (long int)stop_analysis_period_->get()) +
                              " ms.";
            // stop detecting in the ripple detector
            analysis_unlocked_->set(false);

            // buzy sleep has no detections should be received and not
            // stimulation should be sent during this time.
            ...
            while (!delayed_event_queue_.empty() and
                ...
                LOG(DEBUG) << name() << "The stimulation of this "
                           << delayed_event_queue_.top().data_in->event()
                           << " has been locked-out due to the detection "
                              "lockout after stimulation.";
                delayed_event_queue_.pop();
            }
        ...
        if (!disabled_->get()) {
            ...
            if (delayed_event_->get()) {
                ...
                LOG(INFO) << name() << ". Save an event (" << data_in->event()
                          << ") to send later with " << wait_time
                          << "ms delayed.";
                auto delay = data_in->source_timestamp() +
                             std::chrono::milliseconds(wait_time);

                if ((not start_after_stimulation_() or
                     not to_lock_out_in_future(delay)) and
                    (not start_after_detection_() or not to_lock_out())) {
                    ...
                    for (auto time_to_start : when_stop_analysis_period_()) {
                        Delayed event_lockout(
                            delay + std::chrono::milliseconds(time_to_start),
                            data_in);
                        lockout_queue_.push(event_lockout);
                    }

                } else {
                    LOG(DEBUG) << name() << data_in->event()
                               << " has been locked-out in delayed mode";
                    ++event_lockout_;
                }
                // If stimulation is sent at the same time as detection = ontime
                // mode
            } else {
                ++ontime_received_event_;
                if (not(start_after_detection_() and
                        start_after_stimulation_()) or
                    not to_lock_out()) {
                    send_event(data_in, msg_ontime_());
                    for (auto time_to_start : when_stop_analysis_period_()) {
                        Delayed event_lockout(
                            data_in->source_timestamp() +
                                std::chrono::milliseconds(time_to_start),
                            data_in);
                        lockout_queue_.push(event_lockout);
                    }
                } else {
                    LOG(DEBUG) << name() << data_in->event()
                               << " has been locked-out in ontime mode";
                    ++event_lockout_;
                }
            }
            ...
            if (not start_after_detection_() or not to_lock_out()) {
                ...
                LOG(DEBUG) << name() << data_in->event()
                           << " has been locked-out in disable mode";
                ++event_lockout_;
            }
        }
        data_in_port_->slot(0)->ReleaseData();
...
bool EventDelayed::to_lock_out_in_future(TimePoint start_event) {
    ...
    auto millis = std::chrono::duration_cast<std::chrono::milliseconds>(
                      start_event - last_future_event)
                      .count();

    if (millis <= stop_detection_period_->get()) {
        LOG(DEBUG) << name()
                   << ". Start a stimulation lockout after stimulation for " +
                          std::to_string(stop_detection_period_->get()) +
                          " secs.";
        return true;
    }
...
bool EventDelayed::to_lock_out() {
    ...
    if (millis <= stop_detection_period_->get()) {
        LOG(DEBUG) << name()
                   << ". Start a stimulation lockout after detection for " +
                          std::to_string(stop_detection_period_->get()) +
                          " secs.";
        return true;
    }
