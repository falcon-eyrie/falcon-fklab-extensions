...
EventDelayed::EventDelayed() : delayed_range_(150, 200) {
    add_option(DISABLED_S, default_disabled_, "Enable the processing of incoming events.");
...
void EventDelayed::Configure(const GlobalContext& context) {
    ...
    if (!initial_delayed_event_()) {
        ...
        LOG(INFO) << name() << ". Events are delayed of a range between" << delayed_range_.lower()
                  << " and " << delayed_range_.upper() << " ms.";
    }
...
void EventDelayed::CreatePorts() {
    data_in_port_ =
        create_input_port<EventType>(EventType::Capabilities(), PortInPolicy(SlotRange(1), false));

    output_port_ = create_output_port<EventType>(EventType::Parameters(DEFAULT_EVENT),
                                                 PortOutPolicy(SlotRange(1)));
    // -----  Mode state --- //
    disabled_ = create_follower_state(DISABLED_S, default_disabled_(), Permission::WRITE);

    delayed_event_ = create_follower_state(DELAYED_S, initial_delayed_event_(), Permission::WRITE);
    ...
    while (!context.terminated()) {
        ...
        while (!lockout_queue_.empty() and lockout_queue_.top().ts < Clock::now()) {
            LOG(DEBUG) << name()
                       << ". Start a lockout after stimulation for " +
                              std::to_string((long int) stop_analysis_period_->get()) + " ms.";
        ...
        if (!disabled_->get()) {
            ...
            if (delayed_event_->get()) {
                ...
                auto delay = data_in->source_timestamp() + std::chrono::milliseconds(wait_time);

                if ((not start_after_stimulation_() or not to_lock_out_in_future(delay)) and
                    ...
                    for (auto time_to_start : when_stop_analysis_period_()) {
                        Delayed event_lockout(delay + std::chrono::milliseconds(time_to_start),
                                              data_in);
                ...
                if (not(start_after_detection_() and start_after_stimulation_()) or
                    ...
                    for (auto time_to_start : when_stop_analysis_period_()) {
                        Delayed event_lockout(
                            data_in->source_timestamp() + std::chrono::milliseconds(time_to_start),
                            data_in);
                        lockout_queue_.push(event_lockout);
                    }
                } else {
...
void EventDelayed::Postprocess(ProcessingContext& context) {
    auto msg =
        "Successfully executed conversion protocol: " + std::to_string(ontime_received_event_) +
...
bool EventDelayed::to_lock_out_in_future(TimePoint start_event) {
    ...
    auto millis =
        std::chrono::duration_cast<std::chrono::milliseconds>(start_event - last_future_event)
            .count();
...
bool EventDelayed::to_lock_out() {
    auto millis =
        std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() - previous_TS_nostim_)
            .count();
