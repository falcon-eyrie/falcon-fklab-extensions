...
void EventDelayed::CreatePorts() {
  ...
  // -----  Mode state --- //
  disabled_ =
      create_follower_state(DISABLED_S, default_disabled_(), Permission::WRITE);

  delayed_event_ = create_follower_state(DELAYED_S, initial_delayed_event_(),
                                         Permission::WRITE);

  // ----- Lock-out state --- //
  stop_detection_period_ = create_static_state(STOP_DETECTION_TIME_S,
                                               initial_stop_detection_period_(),
                                               true, Permission::WRITE);

  stop_analysis_period_ =
      create_static_state(STOP_ANALYSIS_TIME_S, initial_stop_analysis_period_(),
                          true, Permission::WRITE);
  ...
  while (!context.terminated()) {
    ...
    while (!delayed_event_queue_.empty() and
      ...
      LOG(DEBUG) << name() << ". Time to sent a delayed event ("
                 << delayed_event_queue_.top().data_in->event() << ") with "
                 << millis << "ms late.";
    ...
    while (!lockout_queue_.empty() and lockout_queue_.top().ts < Clock::now()) {
      LOG(DEBUG) << name()
                 << ". Start a lockout after stimulation for " +
                        std::to_string((long int)stop_analysis_period_->get()) +
                        " ms.";
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds((long int)stop_analysis_period_->get()));
    ...
    if (!disabled_->get()) {
      ...
      if (delayed_event_->get()) {
        ...
        if ((not start_after_stimulation_() or
          ...
          for (auto time_to_start : when_stop_analysis_period_()) {
            Delayed event_lockout(
                delay + std::chrono::milliseconds(time_to_start), data_in);
        ...
        if (not(start_after_detection_() and start_after_stimulation_()) or
            not to_lock_out()) {
          send_event(data_in, msg_ontime_());
          for (auto time_to_start : when_stop_analysis_period_()) {
            Delayed event_lockout(data_in->source_timestamp() +
  ...
  if (millis <= stop_detection_period_->get()) {
    LOG(DEBUG) << name()
               << ". Start a stimulation lockout after stimulation for " +
                      std::to_string(stop_detection_period_->get()) + " secs.";
    return true;
  }
