...
struct Delayed {
    ...
    Delayed(TimePoint ts, EventType::Data *data_in)
        : ts(ts), data_in(data_in) {}
...
class EventDelayed : public IProcessor {
  ...
  public:
    ...
    /* Use with delayed event check if enough time will have passed between this
     * stimulation and the last occuring stimulation. if still in the look out
  ...
  protected:
    ...
    options::Value<std::vector<long int>, true> initial_delayed_range_{
        {150, 200}};
