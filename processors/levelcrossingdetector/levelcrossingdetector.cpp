...
void LevelCrossingDetector::CreatePorts() {
    ...
    upslope_ = create_static_state(UPSLOPE, initial_upslope_(), true,
                                   Permission::WRITE);

    post_detect_block_ =
        create_static_state(POST_DETECT_BLOCK, initial_post_detect_block_(),
                            true, Permission::WRITE);
}

void LevelCrossingDetector::CompleteStreamInfo() {
    data_out_port_->streaminfo(0).set_stream_parameters(
        IRREGULARSTREAM, data_in_port_->streaminfo(0).stream_name());
}
