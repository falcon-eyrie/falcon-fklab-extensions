...
LevelCrossingDetector::LevelCrossingDetector() : IProcessor() {
    ...
    add_option(POST_DETECT_BLOCK, initial_post_detect_block_, "Refractory period after threshold",
...
void LevelCrossingDetector::CreatePorts() {
    ...
    data_out_port_ = create_output_port<EventType>(EVENTDATA, EventType::Parameters(),
                                                   PortOutPolicy(SlotRange(1)));

    threshold_ = create_static_state(THRESHOLD, initial_threshold_(), true, Permission::WRITE);

    upslope_ = create_static_state(UPSLOPE, initial_upslope_(), true, Permission::WRITE);

    post_detect_block_ = create_static_state(POST_DETECT_BLOCK, initial_post_detect_block_(), true,
                                             Permission::WRITE);
}
...
void LevelCrossingDetector::Preprocess(ProcessingContext& context) {
    ...
    previous_sample_.assign(data_in_port_->prototype(0).ncolumns(), init_value);
}
...
void LevelCrossingDetector::post_detection_block_update(unsigned int post_detection_block) {
    double post_detection_block_us =
        post_detection_block / data_in_port_->prototype(0).sample_rate() * 1e6;

    LOG(INFO) << name() << ". Post-detection block is set to " << post_detection_block_us
              << " microseconds.";
