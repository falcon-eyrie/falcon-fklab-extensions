...
void BurstDetector::CreatePorts() {
    data_in_port_ =
        create_input_port<MUAType>("mua", MUAType::Capabilities(), PortInPolicy(SlotRange(1)));

    data_out_port_ = create_output_port<EventType>(EVENTDATA, EventType::Parameters("burst"),
...
void BurstDetector::CompleteStreamInfo() {
    stats_nsamples_ = stats_buffer_size_() * 1e3 / data_in_port_->prototype(0).bin_size();
    ...
    stats_out_port_->streaminfo(0).set_parameters(TimeSeriesType<double>::Parameters(
        N_STATS_OUT, stats_nsamples_, 1. / data_in_port_->prototype(0).bin_size()));
...
void BurstDetector::Preprocess(ProcessingContext& context) {
    ...
    sample_rate_ = 1. / data_in_port_->prototype(0).bin_size() * 1e3;
    ...
    if (burn_in_ == 0) {
        burn_in_ = 1;
        LOG(UPDATE) << name() << ". " << SMOOTH_TIME << " too low. Burn-in set to 1 sample.";
    }
...
void BurstDetector::Process(ProcessingContext& context) {
    ...
    while (!context.terminated()) {
        ...
        if (stats_out_->get()) {
            ...
            stats_out->set_data_sample(stats_nsamples_counter, 1, threshold_detector_->threshold());
            stats_out->set_sample_timestamp(stats_nsamples_counter, data_in->hardware_timestamp());
        ...
        if (threshold_detector_->has_crossed_up(test_value)) {
            block_ =
                static_cast<decltype(block_)>(detection_lockout_time_->get() * sample_rate_ / 1e3);
