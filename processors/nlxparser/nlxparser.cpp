...
NlxParser::NlxParser() : IProcessor(PRIORITY_HIGH) {
    ...
    add_option("gap_fill", gap_fill_,
               ...
               "missed packets will be filled with the last available batch of "
               "samples "
               "at each iteration.");
}
...
void NlxParser::CreatePorts() {
    ...
    output_port_ttl_ = create_output_port<TimeSeriesType<uint32_t>>(
        "ttl", TimeSeriesType<uint32_t>::Parameters(), PortOutPolicy(SlotRange(1), 500));

    n_invalid_ = create_broadcaster_state<uint64_t>(
        "n_invalid", 0, Permission::READ, "The number of invalid packets that were received.");
}

void NlxParser::CompleteStreamInfo() {
    nchannels_ = nlx::NLX_NCHANNELS_FROM_NFIELDS(
        data_in_port_->slot(0)
            ->streaminfo()
            .parameters<const typename VectorType<uint32_t>::Parameters&>()
            .size);
    ...
    output_port_signal_->streaminfo(0).set_parameters(TimeSeriesType<double>::Parameters(
        nchannels_, batch_size_(), data_in_port_->slot(0)->streaminfo().stream_rate()));

    output_port_signal_->streaminfo(0).set_stream_rate(
        data_in_port_->slot(0)->streaminfo().stream_rate() / batch_size_());

    output_port_ttl_->streaminfo(0).set_parameters(TimeSeriesType<uint32_t>::Parameters(
        1, batch_size_(), data_in_port_->slot(0)->streaminfo().stream_rate()));
...
void NlxParser::Process(ProcessingContext& context) {
    ...
    while (!context.terminated()) {
        ...
        if (rc != 0) {
            ...
            LOG(DEBUG) << name()
                       << ". Raw packet id:" << nlxrecord_.buffer_[nlx::NLX_FIELD_RAWPACKETID]
                       << " instead of " << nlx::NLX_RAWPACKETID;
            LOG(DEBUG) << name() << ". Packet size field: "
                       << " \nReported size in the packet: "
                       << nlxrecord_.buffer_[nlx::NLX_FIELD_PACKETSIZE]
...
void NlxParser::Postprocess(ProcessingContext& context) {
    ...
    LOG(UPDATE) << name() << ". Finished reading : " << valid_packet_counter_
                << " packets received over " << static_cast<double>(runtime.count()) / 1000
...
void NlxParser::print_stats(bool condition) {
    LOG_IF(UPDATE, condition) << name() << ". Stats report: " << stats_.n_invalid << " invalid, "
                              ...
                              << stats_.n_gaps << " gaps. " << n_filling_packets_
