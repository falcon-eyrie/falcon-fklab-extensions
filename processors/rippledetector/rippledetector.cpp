...
void RippleDetector::CreatePorts() {
  ...
  event_out_port_ = create_output_port<EventType>(
      EVENTDATA, EventType::Parameters("ripple"), PortOutPolicy(SlotRange(1)));
    ...
    for (unsigned int sample = 0; sample < data_in->nsamples(); ++sample) {
      ...
      if (stats_out_->get()) {
        ...
        if (stats_skip_counter == 0) {
          stats_out->set_data_sample(stats_nsamples_counter, "statistics",
                                     test_value);
...
inline double RippleDetector::compute_value(
