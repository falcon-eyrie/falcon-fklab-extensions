 ...
 * @brief High-performance neural signal processor for real-time sharp-wave ripple (SWR) detection.
 *
 * Designed for closed-loop electrophysiology applications. Tracks ongoing local field
 * potential (LFP) dynamics, computes running baseline statistics (mean and Median Absolute
 * Deviation) to establish adaptive thresholds, and triggers low-latency events upon detecting
 * high-frequency, high-amplitude transient ripple oscillations.
 */
