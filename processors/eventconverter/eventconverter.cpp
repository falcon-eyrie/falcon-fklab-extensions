...
EventConverter::EventConverter() {
    add_option("event_name", event_name_, "New event name after conversion");
...
void EventConverter::CreatePorts() {
    ...
    data_out_port_ = create_output_port<EventType>(EventType::Parameters(DEFAULT_EVENT),
...
void EventConverter::Postprocess(ProcessingContext& context) {
    LOG(INFO) << name() << ". Streamed " << data_in_port_->slot(0)->status_read() << " events";
}
