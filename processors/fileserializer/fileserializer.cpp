...
void FileSerializer::Preprocess(ProcessingContext& context) {
    ...
    for (int k = 0; k < data_port_->number_of_slots(); k++) {
        ...
        LOG(DEBUG) << "Successfully opened output file for stream " << std::to_string(k) << " ("
                   << address << ").";

        upstream_buffer_size_[k] =
            static_cast<unsigned int>(data_port_->slot(k)->upstream_policy().buffer_size());
    }
}
...
void FileSerializer::Postprocess(ProcessingContext& context) {
    ...
    for (SlotType k = 0; k < data_port_->number_of_slots(); k++) {
        if (nskipped_[k] != 0) {
            ...
            LOG(UPDATE) << name() << ": stream " << k << ": received and saved " << packetid_[k]
                        << " data packets.";
        }
    }
}
