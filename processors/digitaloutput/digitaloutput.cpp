...
void DigitalOutput::Configure(const GlobalContext& context) {
    if (device_type_() == "dummy") {
        ...
        throw ProcessingConfigureError("No valid digital output device specified.", name());
    }
    ...
    LOG(INFO) << name() << ". There are " << protocols_.size() << " configured output protocols.";
}

void DigitalOutput::CreatePorts() {
    data_in_port_ =
        create_input_port<EventType>(EventType::Capabilities(), PortInPolicy(SlotRange(1)));
}
    ...
    while (!context.terminated()) {
        ...
        if (protocols_.count(data_in->event()) > 0) {
            try {
                ...
                LOG(UPDATE) << name() << ". Protocol executed for " << data_in->event()
                            << " event.";
            } catch (DigitalDeviceError& e) {
                LOG(WARNING) << ". Could not execute protocol for event (" << data_in->event()
                             << ") : " << e.what();
            }
        }
