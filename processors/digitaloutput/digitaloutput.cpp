...
void DigitalOutput::Configure(const YAML::Node &node,
  ...
  print_protocol_execution_updates_ =
      node["print protocol execution updates"]
          .as<decltype(print_protocol_execution_updates_)>(true);
}
...
void DigitalOutput::CreatePorts() {
  ...
  enabled_state_ =
      create_static_state(ENABLED, default_enabled_, true, Permission::WRITE);

  lockout_period_ = create_static_state(LOCKOUT_PERIOD, default_lockout_period_,
...
void DigitalOutput::Preprocess(ProcessingContext &context) {
  ...
  previous_TS_nostim_ =
      std::numeric_limits<decltype(previous_TS_nostim_)>::min();
...
void DigitalOutput::Process(ProcessingContext &context) {
  ...
  while (!context.terminated()) {
    ...
    if (enabled_state_->get() && protocols_.count(data_in->event()) > 0) {
      ...
      if (!to_lock_out(data_in->hardware_timestamp())) {
        try {
          if (context.test()) {
          ...
          LOG_IF(UPDATE, print_protocol_execution_updates_)
              << name() << ". Protocol executed for " << data_in->event()
              << " event.";
        ...
        if (save_stim_events_) {  // save stim events to disk
          ...
          streams_[filename]->write(reinterpret_cast<const char *>(&ts),
                                    sizeof(decltype(ts)));
        }
...
void DigitalOutput::Postprocess(ProcessingContext &context) {
  LOG(INFO) << name() << ". Received " << nreceived_events_
            ...
            << ". " << n_locked_out_events_
            << " executions of the stimulation protocol were locked out.";
...
bool DigitalOutput::to_lock_out(const uint64_t current_timestamp) {
  if (current_timestamp < previous_TS_nostim_) {
    LOG(UPDATE) << name() << "current ts: " << current_timestamp
                << ". previous TS: " << previous_TS_nostim_;

    throw ProcessingError("Non-sequential stimulation event timestamp.",
                          name());
  }
  delta_TS_ = (current_timestamp - previous_TS_nostim_) / 1000;

  if (delta_TS_ <= lockout_period_->get()) {
    return true;
  }
