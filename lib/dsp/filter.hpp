...
class IFilter {
 public:
  ...
  virtual void process_channel(std::vector<double>&,
                               std::vector<double>&,
...
class FirFilter : public IFilter {
 public:
  ...
  static FirFilter* FromStream(std::istream& stream,
                               std::string description,
                               bool binary = false);
  ...
  void process_channel(uint64_t nsamples,
  ...
  void process_by_sample(std::vector<std::vector<double>>& input,
                         std::vector<std::vector<double>>& output) final;
  void process_by_channel(uint64_t nsamples,
                          double** input,
                          double** output) final;
  void process_by_sample(uint64_t nsamples,
                         double** input,
                         double** output) final;
...
class SlopeFilter : public FirFilter {
 public:
  SlopeFilter(uint32_t window_size,
              uint32_t order,
              uint32_t derivative_order,
              std::string description = "")
      : FirFilter(gram_sg::compute_weights(window_size,
                                           window_size,
                                           order,
                                           derivative_order),
                  description),
