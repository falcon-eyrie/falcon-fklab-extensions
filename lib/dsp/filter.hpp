...
class IFilter {
   public:
    ...
    // virtual void process_sample(std::vector<double> input, std::vector<double> output) = 0;
...
class FirFilter : public IFilter {
   public:
    ...
    // all channels, single sample
    void process_sample(std::vector<double>& input, std::vector<double>& output) final;
    ...
    void process_channel(uint64_t nsamples, std::vector<double>::iterator input,
                         std::vector<double>::iterator output, unsigned int channel = 0);
    ...
    void process_by_sample(std::vector<std::vector<double>>& input,
                           std::vector<std::vector<double>>& output) final;
    void process_by_channel(uint64_t nsamples, double** input, double** output) final;
    void process_by_sample(uint64_t nsamples, double** input, double** output) final;
...
class SlopeFilter : public FirFilter {
   public:
    SlopeFilter(uint32_t window_size, uint32_t order, uint32_t derivative_order,
        ...
        : FirFilter(gram_sg::compute_weights(window_size, window_size, order, derivative_order),
                    description),
...
class BiquadFilter : public IFilter {
   public:
    ...
    void process_channel(std::vector<double>& input, std::vector<double>& output,
                         unsigned int channel = 0) final;
    void process_channel(uint64_t nsamples, std::vector<double>::iterator input,
                         std::vector<double>::iterator output, unsigned int channel = 0) final;
    void process_channel(uint64_t nsamples, double* input, double* output,
                         unsigned int channel = 0) final;
