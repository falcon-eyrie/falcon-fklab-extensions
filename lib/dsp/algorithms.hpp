...
class RunningStatistics {
  public:
    RunningStatistics(double alpha, uint64_t burn_in = 0,
                      bool outlier_protection = false,
                      double outlier_zscore = 3, double outlier_half_life = 1,
                      double center = 0.0, double dispersion = 0.0);
...
class SpikeDetector {
  public:
     ...
     * - the timestamp of the detected spike corresponds to that one of the
     * first sample that crossed the threshold first (independently on whether
     * that sample belongs to the current or previous buffers)
     ...
     * one or more channels no peaks were found, the values of the signals at
     * the threshold-crossing sample will be returned.
     */
    ...
    bool is_spike(const uint64_t timestamp, const ForwardIterator sample) {
        ...
        if (detection_mode_ == SpikeDetectionMode::THRESHOLD) {
            ...
            for (c = 0; c < nchannels_; ++c) {
                ...
                if (previous_sample <= threshold_ &&
                    current_sample > threshold_) {
            ...
            if (peak_countdown_ == 0 || npeaks_found_ == nchannels_) {
                // QUESTION: should we wait until peak_life_time window is over,
                // or restart threshold detection as soon as a peak was found on
                // all channels

                // set spike amplitude of channels without peak
                // QUESTION: should we only accepts spikes with peaks in all
  ...
  private:
    ...
    void update_slope(ForwardIterator sample) {
        for (decltype(nchannels_) channel = 0; channel < nchannels_;
             ++channel) {
