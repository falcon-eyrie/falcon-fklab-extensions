    ...
    while (!done) {
        ...
        if (line.compare(0, 1, "#") != 0) {
            throw std::runtime_error(
                "Invalid line in header (missing \"#\" "
...
IFilter* dsp::filter::construct_from_yaml(const YAML::Node& node) {
    ...
    // coefficients: list of doubles (fir) or list of lists of 6 doubles
    // (biquad) window size : 1 uint (slope filter only) order : 1 uint (slope
    // filter only) derivative order: 1 uint (slope filter only) description:
    // text
    ...
    if (filter_type == "fir") {
        ...
        uint32_t window_size =
            node["windows size"].as<unsigned int>(SlopeFilter::DEFAULT_WINDOW_SIZE);
        uint8_t derivative_order =
            node["derivative order"].as<unsigned int>(SlopeFilter::DEFAULT_ORDER);
...
void FirFilter::process_channel(uint64_t nsamples, std::vector<double>::iterator input,
                                std::vector<double>::iterator output, unsigned int channel) {
    double result;
...
void FirFilter::process_channel(uint64_t nsamples, double* input, double* output,
                                unsigned int channel) {
...
SlopeFilter* SlopeFilter::FromStream(std::istream& stream, std::string description, bool binary) {
    if (binary) {
        throw std::runtime_error("Binary filter files are not yet supported.");
    }
...
BiquadFilter::BiquadFilter(double gain, std::vector<std::array<double, 6>>& coefficients,
...
BiquadFilter* BiquadFilter::FromStream(std::istream& stream, std::string description,
                                       bool binary = false) {
    if (binary) {
        throw std::runtime_error("Binary filter files are not yet supported.");
    }
...
double BiquadFilter::process_channel(double x, unsigned int c) {
    ...
    for (unsigned int s = 0; s < nstages_; ++s) {
        ...
        y_n = coefficients_[s][0] * u_n + coefficients_[s][1] * registers_[c][s][0] +
...
void BiquadFilter::process_channel(std::vector<double>& input, std::vector<double>& output,
                                   unsigned int channel) {
    uint64_t nsamples = input.size();
...
void BiquadFilter::process_channel(uint64_t nsamples, std::vector<double>::iterator input,
                                   std::vector<double>::iterator output, unsigned int channel) {
    for (uint64_t s = 0; s < nsamples; ++s) {
...
void BiquadFilter::process_channel(uint64_t nsamples, double* input, double* output,
                                   unsigned int channel) {
    for (uint64_t s = 0; s < nsamples; ++s) {
        output[s] = process_channel(input[s], channel);
    }
...
void BiquadFilter::process_by_channel(std::vector<std::vector<double>>& input,
                                      std::vector<std::vector<double>>& output) {
    uint64_t nsamples = input.size();
    for (uint64_t s = 0; s < nsamples; ++s) {
        process_sample(input[s], output[s]);
