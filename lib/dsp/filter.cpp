...
std::map<std::string, std::string> dsp::filter::parse_file_header(
    std::istream& stream) {
...
IFilter* dsp::filter::construct_from_yaml(const YAML::Node& node) {
  ...
  if (filter_type == "fir") {
    ...
    uint8_t order =
        node["order"].as<unsigned int>(SlopeFilter::DEFAULT_DERIVATIVE_ORDER);
...
FirFilter* FirFilter::FromStream(std::istream& stream,
                                 std::string description,
                                 bool binary) {
  if (binary) {
...
void FirFilter::process_by_sample(uint64_t nsamples,
                                  double** input,
                                  double** output) {
  for (unsigned int c = 0; c < nchannels_; ++c) {
