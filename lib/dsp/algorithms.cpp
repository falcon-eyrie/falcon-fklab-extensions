...
RunningStatistics::RunningStatistics(double alpha, uint64_t burn_in, bool outlier_protection,
                                     double outlier_zscore, double outlier_half_life, double center,
                                     double dispersion)
...
SpikeDetector::SpikeDetector(unsigned int nchannels, double threshold, unsigned int peak_life_time,
                             SpikeDetectionSign sign)
    : nchannels_(nchannels), threshold_(threshold), peak_life_time_(peak_life_time), sign_(sign) {
    reset();
}
