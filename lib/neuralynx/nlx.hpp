...
const int VTRecNumTransitionBitfields =
    400;  ///< Number of VT bitfield transitions stored in the
          ///< VideoRec::dwPoints array
const int VTRecNumTargets = 50;  ///< Number of VT bitfield transitions stored
                                 ///< in the VideoRec::dntargets array
...
bool valid_nlx_vt(VideoRec* vt_record,
                  std::uint16_t vt_id,
                  ErrorNLXVT::Code& error_code,
                  decltype(NLX_VIDEO_RESOLUTION) resolution);

inline const unsigned int nlx_field_data_last(
    unsigned int nchannels = NLX_DEFAULT_NCHANNELS) {
  return NLX_FIELD_DATA_FIRST + nchannels - 1;
}
inline const unsigned int nlx_field_crc(
    unsigned int nchannels = NLX_DEFAULT_NCHANNELS) {
  return NLX_FIELD_DATA_FIRST + nchannels;
}
inline const int32_t nlx_packetsize(
    unsigned int nchannels = NLX_DEFAULT_NCHANNELS) {
...
class NlxSignalRecord {
 public:
  ...
  template <typename T>
  int FromNetworkBuffer(const std::vector<T>& buffer) {
  ...
  std::vector<int32_t>::iterator data(
      std::vector<int32_t>::iterator it) const;  // will copy
...
uint64_t CheckTimestamp(const NlxSignalRecord& rec,
