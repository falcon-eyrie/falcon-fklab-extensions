...
const int VTRecNumTransitionBitfields = 400;  ///< Number of VT bitfield transitions stored in the
...
typedef struct {                // from Neuralynx header Nlx_DataTypes.h
    ...
    std::uint32_t dwPoints[VTRecNumTransitionBitfields];  ///< An array of bitfields
...
bool valid_nlx_vt(VideoRec* vt_record, std::uint16_t vt_id, ErrorNLXVT::Code& error_code,
