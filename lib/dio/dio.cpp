...
typename std::vector<bool>::reference DigitalState::operator[](
    uint32_t channel) {
  return state_[channel];
}
...
std::string DigitalState::to_string(std::string high,
                                    std::string low,
...
std::vector<uint32_t> DigitalOutputProtocol::find_channels(
    DigitalOutputMode mode) {
