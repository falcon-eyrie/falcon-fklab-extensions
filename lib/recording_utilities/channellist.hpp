  ...
  public:
    ...
    void add_channels(std::vector<T> channels,
        ...
        if (labels.size() == 0) {
            ...
            throw std::length_error(
                ". channels and labels should have the same size.");
        } else {
    ...
    void remove_channels(double range_min, double range_max) {
        ...
        auto to_remove = std::remove_if(
            channels_.begin(), channels_.end(), [range_min, range_max](T x) {
    ...
    void remove_channels(std::vector<T> channels) {
        ...
        auto to_remove =
            std::remove_if(channels_.begin(), channels_.end(), [channels](T x) {
                return std::find(channels.begin(), channels.end(), x.first) !=
                       channels.end();
            });
    ...
    void remove_channels(std::vector<std::string> labels) {
        auto to_remove =
            std::remove_if(channels_.begin(), channels_.end(), [labels](T x) {
                return std::find(labels.begin(), labels.end(), x.second) !=
                       labels.end();
            });
    ...
    /**
     * @brief all_in_range check if all the column numbers in this collection
     * are contained in the range given in input.
     * @param range_min
     * @param range_max
     * @return
     */
    ...
    /**
     * @brief is_subset check if the column numbers in this collection is a
     * subset of the column numbers list given in input.
     * @param channels_vec
     * @return
     */
    bool is_subset(std::vector<T> channels_vec) const {
        ...
        return std::includes(channels.begin(), channels.end(), subset.begin(),
                             subset.end());
    }

    /**
     * @brief is_subset check if the labels in this collection is a subset of
     * the labels list given in input.
     * @param labels_vec
     * @return
     */
    bool is_subset(std::vector<std::string> labels_vec) const {
        const auto subset_vec = get_labels();
        const std::set<std::string> subset(subset_vec.begin(),
                                           subset_vec.end());
        const std::set<std::string> labels(labels_vec.begin(),
                                           labels_vec.end());
        return std::includes(labels.begin(), labels.end(), subset.begin(),
                             subset.end());
    }

    /**
     * @brief is_sorted check without modifying the collection if it is sorted
     * by column numbers or by labels
     *
     * @param by_label  optional, switch the sorting test to sort by labels
    ...
    /**
     * @brief is_unique check without modifying the collection if there is an
     * unique set of column numbers.
     * @return
     */
    bool is_unique() const {
        auto channels = get_channels();
        std::sort(channels.begin(), channels.end());
        return std::adjacent_find(channels.begin(), channels.end()) ==
               channels.end();
    }
     ...
     *
     * std::pair has a sort implementation by default - it sorts automatically
     * by the first element (channel number) and then in case of equality by the
     * second element (label).
     *
     * @param by_label optional, optional, switch the sorting to sort by labels
     * instead of column numbers
     *
     */
    ...
    /**
     * @brief unique - Remove duplicates to keep only an unique set of channel
     * numbers. Because of the way sort works, the first label in alphabetical
     * order is the one keep.
     *
     * No need to have an unique implementation for the labels as we cannot from
     * the start have duplicate labels.
     */
    void unique() {
        sort();
    ...
    void add_element(T channel, std::string label) {
        ...
        if (std::any_of(channels_.begin(), channels_.end(),
            ...
            throw std::invalid_argument(
                ". This label " + label +
                " already exist in this channel list and should be unique");
        }
            ...
            for (auto part : channels) {
                ...
                if (range.size() == 1) {
                    ...
                    throw std::runtime_error(
                        part + " is not a valid list specification.");
                }
            }
        } else {
            return false;
