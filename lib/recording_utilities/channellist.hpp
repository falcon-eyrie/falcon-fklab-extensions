...
class ChannelList {
   public:
    ...
    std::vector<T> get_channel_numbers() const {
        ...
        std::transform(channels_.begin(), channels_.end(), std::back_inserter(channel_numbers),
    ...
    std::vector<std::string> get_labels() const {
        ...
        std::transform(channels_.begin(), channels_.end(), std::back_inserter(labels),
    ...
    void remove_channels(double range_min, double range_max) {
        auto to_remove = std::remove_if(
            ...
            [range_min, range_max](T x) { return x.first >= range_min and x.first <= range_max; });

        channels_.erase(to_remove, channels_.end());
    }

    void remove_channels(std::vector<T> channels) {
        auto to_remove = std::remove_if(channels_.begin(), channels_.end(), [channels](T x) {
    ...
    void remove_channels(std::vector<std::string> labels) {
        auto to_remove = std::remove_if(channels_.begin(), channels_.end(), [labels](T x) {
   ...
   private:
    void add_element(T channel, std::string label) {
        if (std::any_of(
                channels_.begin(), channels_.end(),
                [&label](const std::pair<T, std::string>& ch) { return ch.second == label; })) {
            throw std::invalid_argument(". This label " + label +
