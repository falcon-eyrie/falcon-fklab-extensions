/*!
 \file    serialib.cpp
 \brief   Source file of the class serialib. This class is used for
...
/*!
     ...
     \param Device : Port name (COM1, COM2, ... for Windows ) or (/dev/ttyS0,
   /dev/ttyACM0, /dev/ttyUSB0 ... for linux)
...
/*!
     ...
     \param     receivedString : string to send on the port (must be terminated
   by '\0')
     \return     1 success
     \return    -1 error while writting data
  */
char serialib::writeString(const char *receivedString) {
    ...
    DWORD dwBytesWritten;
    // Write the string
    if (!WriteFile(hSerial, receivedString, strlen(receivedString),
                   &dwBytesWritten, NULL))
...
/*!
  ...
  */
int serialib::readStringNoTimeOut(char *receivedString, char finalChar,
    ...
    while (NbBytes < maxNbBytes) {
        ...
        if (charRead == 1) {
            ...
            // The character is not the final char, increase the number of bytes
            // read
            NbBytes++;
        }
...
/*!
   ...
   (optional)
     ...
     \return -3 MaxNbBytes is reached
  */
int serialib::readString(char *receivedString, char finalChar,
                         unsigned int maxNbBytes, unsigned int timeOut_ms) {
    ...
    while (nbBytes < maxNbBytes) {
        ...
        if (timeOutParam > 0) {
            // Wait for a byte on the serial link with the remaining time as
            // timeout
            ...
            if (charRead == 1) {
                ...
                // This is not the final character, just increase the number of
                // bytes read
                nbBytes++;
            }
...
/*!
     ...
     \param maxNbBytes : maximum allowed number of bytes read
     \param timeOut_ms : delay of timeout before giving up the reading
     \param sleepDuration_us : delay of CPU relaxing in microseconds (Linux
   only) In the reading loop, a sleep can be performed after each reading This
   allows CPU to perform other tasks
     ...
     \return -2 error while reading the byte
  */
int serialib::readBytes(void *buffer, unsigned int maxNbBytes,
