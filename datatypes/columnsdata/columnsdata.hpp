...
class Data : public IData<Data<T>, ParentType> {
   public:
    ...
    /**
     * @brief Data constructor with the labels for each column and the number of
     * samples
    ...
    void set_data_column(std::string column, const std::vector<T>& data) {
        if (data.size() != this->nsamples_) {
            throw std::length_error(
                ". the data vector to set should have the "
                "same size as the number of columns (" +
    ...
    /**
     * @brief set_data_column set all samples for one column based on the column
     * index
    ...
    void set_data_sample(size_t sample, const std::vector<T>& data) {
        if (data.size() != ncolumns_) {
            throw std::length_error(
                ". the data vector to set should have the same size as the "
    ...
    /**
     * @brief set_data_sample set one data for one column index and one sample
     * index
     * @param sample - selected sample index
     * @param column - selected column index
     * @param data
    ...
    /**
     * @brief set_data_sample set one data for one column label and one sample
     * index
     * @param sample - selected sample index
    ...
    /**
     * @brief data - return full data with this format [c1s1, c1s2.., c1sn,
     * c2s1, c2s2 ..., cns1,...cnsn]
     *
     * @return
     */
    ...
    /**
     * @brief end_column - iterator at the end of the column based on the
     * location in the dataset
     * @param column - column index
     * @return
     */
    ...
    /**
     * @brief end_column - iterator at the end of the column based on name of
     * the column
     * @param column - column label
     * @return
     */
    ...
    /**
     * @brief SerializeFlatBuffer  - add to the flexbuffer the data labeled
     * [label0:nsamples, label1:nsamples...], the number of columns and the
     * number of samples.
     * @param flex_builder
     */
    void SerializeFlatBuffer(flexbuffers::Builder& flex_builder) override {
        ...
        flex_builder.TypedVector("data", [&] {
            for (auto samples : data_) flex_builder.Add(samples);
        });
    ...
    /**
     * @brief YAMLDescription - this datatype add to the metadata describing the
     * data format the signal, number of columns and samples format.
     *
     * @param format
     */
    ...
    /**
     * @brief sum_column - compute the (absolute) sum of all samples in the same
     * column
     *
     * @param column - column label
    ...
    /**
     * @brief sum_sample - compute the (absolute) sum of all column data for the
     * same sample index
     *
    ...
    /**
     * @brief mean_sample - compute the mean of the (absolute) sum of all column
     * data for the same sample index
     *
     * @param sample - sample index
     * @param absolute - absolute sum or not
     * @return  the mean of the (absolute) sum
     */
    T mean_sample(size_t sample, bool absolute = false) const {
        return sum_sample(sample, absolute) / ncolumns_;
    }
   ...
   private:
    size_t extract_index_from_column(std::string column) const {
        ...
        if (it == labels_.end()) {
            throw std::runtime_error(". The column " + column + " does not exist in this dataset.");
        }
   ...
   public:
    ...
    void Validate(const Data<T>& prototype) {
        if (!sample_range_.inrange(prototype.nsamples())) {
            throw std::runtime_error("Number of samples cannot be zero and needs to be in range " +
                                     sample_range_.to_string());
        }

        if (!column_range_.inrange(prototype.ncolumns())) {
            throw std::runtime_error("Number of columns cannot be zero and needs to be in range " +
...
template <typename T>
using ColumnsType =
    DefineType<nsColumn::Data<T>, AnyType, true, nsColumn::Capabilities, nsColumn::Parameters>;
