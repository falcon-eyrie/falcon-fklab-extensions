  ...
  public:
    ...
    void set_data_column(std::string column, const std::vector<T> &data) {
        ...
        if (data.size() != this->nsamples_) {
            throw std::length_error(". the data vector to set should have the "
    ...
    void set_data_sample(size_t sample, size_t column, T data) {
        ...
        if (column >= ncolumns_) {
            throw std::out_of_range(
                ". Channel index " + std::to_string(sample) +
    ...
    void SerializeBinary(std::ostream &stream,
        ...
        if (format == Serialization::Format::COMPACT) {
            for (size_t k = 0; k < nsamples_; ++k) {
                stream.write(
                    reinterpret_cast<const char *>(&data_[flat_index(k)]),
                    sizeof(T) * ncolumns_);
            }
        }
    }
...
class Capabilities {
  public:
    Capabilities(ChannelRange column_range,
        ...
        : column_range_(column_range), sample_range_(sample_range),
          labels_({}) {}
...
template <typename T>
using ColumnsType = DefineType<nsColumn::Data<T>, AnyType, true,
