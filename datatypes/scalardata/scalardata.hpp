...
template <typename TYPE> struct Parameters {
...
template <typename TYPE> class Data : public IData<Data<TYPE>, Base> {
  public:
    ...
    ScalarData(const Parameters<TYPE> &parameters)
        : ScalarData(parameters.default_value) {}
...
template <typename T>
using ScalarType =
    DefineType<nsScalarType::Data<T>, AnyType, true, nsScalarType::Capabilities,
               nsScalarType::Parameters<T>>;
