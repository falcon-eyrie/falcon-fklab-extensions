...
class Data : public IData<Data<T>, ParentType> {
 public:
  ...
  void setData(const std::vector<T>& data) {
    if (data.size() != data_.size()) {
      throw std::runtime_error(
          "Setting vector data from wrong size source vector");
    }
    data_ = data;  // copy
  }
  ...
  void setData(const T* data, int len) {
    if (len != data_.size()) {
      throw std::runtime_error(
          "Setting vector data from wrong size source data");
    }
...
template <typename T>
using VectorType = DefineType<nsVectorType::Data<T>,
                              AnyType,
                              true,
                              nsVectorType::Capabilities,
