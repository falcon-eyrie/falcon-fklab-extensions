...
class Data : public IData<Data, ParentType> {
 public:
  ...
  Data(unsigned int nchannels,
       double buffer_size,
       double sample_rate,
       size_t max_nspikes = 0);

  Data(const Parameters& parameters)
      : Data(parameters.nchannels,
  ...
  std::vector<double>::const_iterator spike_amplitudes(
      std::size_t spike_index) const;
...
using SpikeType = DefineType<nsSpikeType::Data,
                             AnyType,
                             true,
                             nsSpikeType::Capabilities,
