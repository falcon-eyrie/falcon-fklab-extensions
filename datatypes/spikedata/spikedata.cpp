...
Data::Data(unsigned int nchannels,
           double buffer_size,
           double sample_rate,
           size_t max_nspikes)
...
std::vector<double>::const_iterator Data::spike_amplitudes(
...
void Data::SerializeBinary(std::ostream& stream,
  ...
  if (format == Serialization::Format::FULL) {
    ...
    stream.write(
        reinterpret_cast<const char*>(zero_timestamps.data()),
        n_spikes_to_fill_buffer * sizeof(decltype(zero_timestamps[0])));
    stream.write(
        reinterpret_cast<const char*>(amplitudes_.data()),
