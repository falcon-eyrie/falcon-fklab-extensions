...
struct Parameters : nsColumn::Parameters {
  Parameters(size_t nchan = 0, size_t nsamp = 0, double rate = 1.0)
      : nsColumn::Parameters(generate_labels(nchan), nsamp),
        sample_rate(rate) {}

  Parameters(const std::vector<std::string>& labels,
             size_t nsamp = 0,
             double rate = 1.0)
...
class Data : public IData<Data<T>, Base<T>> {
 ...
 public:
   ...
   *
   * @param columns_label give a label for each column in the data / set also
   * the number of columns in the dataset
   * @param nsamples  give the number of samples by column
   * @param sample_rate
   */
  Data(const std::vector<std::string>& labels,
       size_t nsamples,
       double sample_rate)
      : BaseClass(labels, nsamples) {
    if (sample_rate <= 0) {
      throw std::runtime_error(
          "Time Series Data::Data - sample rate "
  ...
  /**
   * @brief Data constructor with only the number of columns, the number of
   * samples and the sample rate
   *
   ...
   *
   * @param parameters - contains already the column labels either
  ...
  void set_sample_timestamps(const std::vector<uint64_t>& t) {
    if (t.size() != this->nsamples_) {
      throw std::length_error(
          ". the timestamps vector to set should have the same size as "
  ...
  /**
   * @brief FlatbufferDescription - This datatype add in the flexbuffer stream
   * the timestamps and the type name.
   *
  ...
  /**
   * @brief SerializeBinary - This datatype add in the binary stream the
   * timestamps.
   *
   ...
   * data format the timestamp format.
   *
   * @param node
   * @param format
   */
...
template <typename T>
using TimeSeriesType = DefineType<nsTimeSeries::Data<T>,
                                  ColumnsType<T>,
                                  false,
                                  nsTimeSeries::Capabilities,
                                  nsTimeSeries::Parameters>;
