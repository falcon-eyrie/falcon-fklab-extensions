...
SquareSource::SquareSource(double offset, double amplitude, double frequency, double duty_cycle,
                           double sampling_rate, double noise_stdev, unsigned int nchannels,
...
std::string SquareSource::string() {
    return "square wave (fs = " + to_string_n(sampling_rate_) + " Hz, " +
           "offset = " + to_string_n(offset_) + " uV, " + "amplitude = " + to_string_n(amplitude_) +
           " uV, " + "frequency = " + to_string_n(frequency_) +
           " Hz, "
           "duty cycle = " +
           to_string_n(duty_cycle_ * 100) + "%, " + "noise stdev = " + to_string_n(noise_stdev_) +
           " uV, " + "number of channels = " + std::to_string(nchannels_) + ", " +
...
SquareSource* SquareSource::from_yaml(const YAML::Node node) {
    return new SquareSource(
        ...
        node["sampling_rate"].as<double>(32000), node["noise_stdev"].as<double>(0),
        node["nchannels"].as<unsigned int>(128), node["convert_byte_order"].as<bool>(true));
}
