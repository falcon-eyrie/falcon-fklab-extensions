...
SineSource::SineSource(double offset, double amplitude, double frequency, double sampling_rate,
                       double noise_stdev, unsigned int nchannels, bool convert_byte_order)
    : offset_(offset),
...
std::string SineSource::string() {
    return "sine wave (fs = " + to_string_n(sampling_rate_) + " Hz, " +
           ...
           " uV, " + "frequency = " + to_string_n(frequency_) + " Hz, " +
...
SineSource* SineSource::from_yaml(const YAML::Node node) {
    return new SineSource(
        ...
        node["frequency"].as<double>(1.0), node["sampling_rate"].as<double>(32000),
        node["noise_stdev"].as<double>(0), node["nchannels"].as<unsigned int>(128),
