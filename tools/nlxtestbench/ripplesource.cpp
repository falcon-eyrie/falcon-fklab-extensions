...
RippleSource::RippleSource(double offset, double frequency,
                           double sampling_rate, YAML::Node ripple_duration,
                           double mean_amplitude, double noise_stdev,
                           bool convert_byte_order, unsigned int nchannels)
    ...
    for (unsigned int i = 0; i < nchannels; i++) {
        ...
        if (ripple_duration[i]) {
            param.interval = ripple_duration[i]["interval"].as<double>() *
                             sampling_rate_ / 1000;
            param.duration = ripple_duration[i]["duration"].as<double>() *
                             sampling_rate_ / 1000;
        } else {
            param.interval = ripple_duration["*"]["interval"].as<double>() *
                             sampling_rate_ / 1000;
            param.duration = ripple_duration["*"]["duration"].as<double>() *
                             sampling_rate_ / 1000;
        }
...
std::string RippleSource::string() {
    ...
    for (auto it = ripple_params_.begin(); it != ripple_params_.end(); ++it) {
        ...
        if (key.as<std::string>() == "*") {
            general_ripples += "\nFor all channels : ";
            general_ripples +=
                "mean ripple amplitude = " + to_string_n(mean_amplitude_) +
                " uV, " + "ripple frequency = " + to_string_n(frequency_) +
                " Hz, " +
                "ripple duration = " + value["duration"].as<std::string>() +
                " ms, " + "zero signal interval = " +
                value["interval"].as<std::string>() + " ms ";
        } else {
