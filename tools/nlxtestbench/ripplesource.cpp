...
RippleSource::RippleSource(   double offset, double amplitude, double frequency,
    double duration, double interval, double sampling_rate, double noise_stdev, unsigned int nchannels,
    bool convert_byte_order ) :

    offset_(offset), amplitude_(amplitude), frequency_(frequency), duration_(duration),
    interval_(interval), sampling_rate_(sampling_rate), noise_stdev_(noise_stdev), delta_(1000000/sampling_rate),
    distribution_(0.0, noise_stdev), ripple_(false), poisson_distribution_(amplitude), nchannels_(nchannels), convert_byte_order_(convert_byte_order) {

    if (frequency_<150 || frequency_>250) {
        throw std::runtime_error("Invalid frequency for a SWRs - should be between 150 and 250 Hz");
    }


    duration_ = duration_ * sampling_rate_/10;
    interval_ = interval_ * sampling_rate_/10;

    counter_ = 0;
    omega_ = 2 * 3.14159265358979 * frequency_;
...
std::string RippleSource::string() {
    ...
    return "ripple wave (fs = " + to_string_n(sampling_rate_) + " Hz, " +
        ...
        "ripple duration = " + to_string_n(duration_*10/sampling_rate_) + " secs, " +
        "zero signal duration = " + to_string_n(interval_*10/sampling_rate_) + " secs, " +
...
int64_t RippleSource::Produce( char** data ) {
    ...
    if (!ripple_ and counter_> interval_){ //starting a ripple at the end of the interval segment
    ...
    if (ripple_ and counter_ < duration_){ //generating ripple values
            current_amplitude_ = amplitude_*std::exp(-0.5*std::pow(4*counter_/duration_, 2))*std::cos( counter_*omega_ ) ;
    } else{     // interval segment = amplitude of the signal = 0
        current_amplitude_ = 0;

        if (ripple_){   // starting an interval segment at the end of the ripple signal
            counter_ = 0;
            ripple_ = false;
        }
...
RippleSource* RippleSource::from_yaml( const YAML::Node node ) {
    ...
    return new RippleSource( node["offset"].as<double>(0.0),
                             node["amplitude"].as<double>(1.0),
                             node["frequency"].as<double>(200),
                             node["duration"].as<double>(10),
                             node["interval"].as<double>(10),
