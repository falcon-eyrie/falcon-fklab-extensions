...
RippleSource::RippleSource(double offset, double frequency, double sampling_rate,
                           YAML::Node ripple_duration, double mean_amplitude, double noise_stdev,
                           bool convert_byte_order, unsigned int nchannels)
    : offset_(offset),
      frequency_(frequency),
      sampling_rate_(sampling_rate),
      delta_(1000000 / sampling_rate),
      ripple_params_(ripple_duration),
      mean_amplitude_(mean_amplitude),
      poisson_distribution_(mean_amplitude),
...
RippleSource* RippleSource::from_yaml(const YAML::Node node) {
    ...
    return new RippleSource(
        node["offset"].as<double>(0.0), node["ripple frequency"].as<double>(200),
        node["sampling_rate"].as<double>(32000),
        node["ripple param"].as<YAML::Node>(default_ripple_params),
        node["mean ripple amplitude"].as<double>(100), node["noise_stdev"].as<double>(0),
        node["convert_byte_order"].as<bool>(true), node["nchannels"].as<unsigned int>(128));
}