...
std::vector<std::unique_ptr<DataSource>> datasources_from_yaml(const YAML::Node& node) {
    ...
    // node["sources"]
    if (!node || node.IsNull() ||
        (node["class"] and node["class"].as<std::string>() == "default")) {
        ...
        for (YAML::const_iterator it = node.begin(); it != node.end(); ++it) {
            ...
            if (!it->IsMap() || !(*it)["class"]) {
                ...
                if (source_class == "nlx") {
                    sources.push_back(
                        std::unique_ptr<DataSource>(FileSource::from_yaml((*it)["options"])));
                } else if (source_class == "noise") {
                    sources.push_back(
                        std::unique_ptr<DataSource>(WhiteNoiseSource::from_yaml((*it)["options"])));
                } else if (source_class == "sine") {
                    sources.push_back(
                        std::unique_ptr<DataSource>(SineSource::from_yaml((*it)["options"])));
                } else if (source_class == "square") {
                    sources.push_back(
                        std::unique_ptr<DataSource>(SquareSource::from_yaml((*it)["options"])));
                } else if (source_class == "ripple") {
                    sources.push_back(
                        std::unique_ptr<DataSource>(RippleSource::from_yaml((*it)["options"])));
                }
            }
        }
    }
