...
void test_filter_timing(std::unique_ptr<dsp::filter::IFilter> const& filter, unsigned int nchannels,
    ...
    std::cout << "Filtering " << npoints << " points took " << duration << " milliseconds ("
              << (double) duration * 1000 / npoints << " microseconds/point)" << std::endl;
...
int main(int argc, char** argv) {
    ...
    parser.add<std::string>("signal_output", 'o', "output file for filtered signal", false,
                            "filtered_signal.dat");
    ...
    parser.add<std::string>("impulse_output", 'f', "output file for impulse response", false,
    ...
    auto filter =
        std::unique_ptr<dsp::filter::IFilter>(dsp::filter::construct_from_file(parser.rest()[0]));

    std::cout << "filter description: " << filter->description() << std::endl;
    std::cout << "filter order: " << filter->order() << std::endl;

    // 1. filter input signal
    test_filter_signal(parser.get<std::string>("signal"), parser.get<std::string>("signal_output"),
                       filter);
